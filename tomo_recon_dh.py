...
def dataStandardReader(filename, sliceStart=0, sliceEnd=20, flat_name=None, dark_name=None,
    ...
    if ds_use:
        ...
        try:
            ...
            exit()
        white = ds(arr[1:9,sliceStart:sliceEnd,:], ds_level, ds_axis)
        f.close()
        if fake_flat:
            white = ds(fake_flat_val*np.ones([8, white.shape[1], white.shape[2]]), ds_level, ds_axis)
        ...
        try:
            ...
            exit()
        dark = ds(arr[1:9,sliceStart:sliceEnd,:], ds_level, ds_axis)
        f.close()
        if fake_dark:
            dark = ds(fake_dark_val*np.ones([8, dark.shape[1], dark.shape[2]]), ds_level, ds_axis)
        ...
        try:
            ...
            exit()
        data = ds(arr[:,sliceStart:sliceEnd,:], ds_level, ds_axis)
        f.close()
        ...
        try:
            ...
            print ('There is no flat images in the file! Please provide an alternative file with flat images (using argument \'flat_name=some_name\'). Quit.')
            exit()
        white = np.float32(arr[:,sliceStart:sliceEnd,:])
        white[:] = np.where(white<0, white+2**16, white)[:]
        f.close()
        if fake_flat:
        ...
        try:
            ...
            print ('There is no dark images in the file! Please provide an alternative file with dark images (using argument \'dark_name=some_name\'). Quit.')
            exit()
        dark = np.float32(arr[:,sliceStart:sliceEnd,:])
        f.close()
        if fake_dark:
        ...
        try:
            ...
            print('There is no img_tomo in the file! Please provide an alternative file with dark images (using argument \'dark_name=some_name\'). Quit.')
            exit()
        data = np.float32(arr[:,sliceStart:sliceEnd,:])
        data[:] = np.where(data<0, data+2**16, data)[:]
        f.close()
...
#    output_files = '/media/staff/XIAO_USB2/Enyuan_June2019_recon2/' + 'recon_'+os.path.basename(filenames[0]).split(".")[-2]+'/recon_'+os.path.basename(filenames[0]).split(".")[-2]
...
def manualFindCenter(center_shift,center_shift_w,**kwargs):
    ...
    if ds_use.upper() == 'YES':
        ds_level = kwargs['downsample_Filter']['params']['level']
        ds_axis = kwargs['downsample_Filter']['params']['axis']
    else:
        ds_level = None
        ds_axis = None
    ...
    data,white,dark,theta = dataStandardReader(filename,sliceStart=sliceStart,sliceEnd=sliceEnd,
                                               ...
                                               ds_level=ds_level, ds_axis=ds_axis)
    print(white.max(), white.min(), white.shape)
#    data[:] = np.where(data<0, data + 2**16, data)[:]
#    white[:] = np.where(white<0, white + 2**16, white)[:]
    print(white.max(), white.min(), white.shape)
...
#    # remove zingers (pixels with abnormal counts)
    ...
    #tifffile.imsave('/media/Disk2/data/debug/raw_data.tiff',data)
    #tifffile.imsave('/media/Disk2/data/debug/raw_white.tiff',white)
    ...
    #tifffile.imsave('/media/Disk2/data/debug/norm_data.tiff',data)

#    data = filters.gaussian_laplace(data,5)
#    raw_data_top_dir = "/media/Disk2/data/BrendanCroom_2018Q3"
    data = generalFilterContainer(data,**kwargs)
    if data_center_path==None:
        data_center_path = '~/tomopy_data_center'

    #tifffile.imsave('/media/Disk2/data/debug/after_filter.tiff',data)

    if wedge is True:
        print ('Projection images are in less than 180 degree range.')
        data = sortWedge(data,**kwargs)
...
##### This is the only section which you need to edit
    ...
    # scan_idx = [27192, 27193, 27194, 27195, 27196, 27197, 27198, 27199, 27200, 27201, 27202, 27203, 27204, 27205, 27206, 27207, 27208, 27209, 27210, 27211, 27212 ]
  

 
# step 1, single scan_idx to test the reconstruction, decide the offset 
#    scan_idx = [27381]
#    center_list = [628]


# step 1.5, check numRecSlices
#    raw_data_top_dir = "/home/VTLinlab/Documents/DH/Gravel1/Gravel1_200_second"
#    scan_idx = [27570]
#    center_list = [625]
...
#    numRecSlices = None
    ...
    # first find the first and last slice containing the particles, particle_s and particle_e
    # numRecSlices = particle_e - particle_s + total_gap(70) 
    # offset = particle_s - total_gap/2
    # slice start = offset + margin_slices/2
    # slice end = particle_e + total_gap/2 - margin_slices/2



# step 2, batch reconstruction
    raw_data_top_dir = "/run/media/VTLinlab/Lin_Lab_5/Rod_Gravel_BNL_3D/Rod3/Rod3_200"
    # scan_idx = [75178]
    scan_idx = np.arange(27234,27255,1)
    center_list = np.full(21, 625)

    # scan_idx = np.arange(74062, 74044, -1)    # idx_start, idx_end+1
    # center_list = [636,637,635,638,636,640,640,642,643,640,640,640,640,640,643,643,645,644]
    numRecSlices = None    # set to None if you want to reconstruct the whole volume
    offset = None         # set to None if you want to reconstruct the whole volume
   # If you find center values for the list of data sets defined in scan_idx above, you put these center values here in []. The number of values here in [] must be equal to number of items in scan_idx = [], and orders in these two [] must match.
    ...
    margin_slices = 15      # leave this fixed
    zinger = False
